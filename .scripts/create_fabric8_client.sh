...
function git_configure_and_clone() {
    ...
    set +x
    rm -rf /tmp/${GHREPO}/
    echo git clone https://XXXX@github.com/${GHORG}/${SERVICE_NAME}.git --depth=1 /tmp/${SERVICE_NAME}
    git clone https://$(echo ${FABRIC8_HUB_TOKEN}|base64 --decode)@github.com/${GHORG}/${SERVICE_NAME}.git --depth=1 /tmp/${SERVICE_NAME}
    set -x
}
